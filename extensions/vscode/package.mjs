...
const sources = [["package.json", await readFile("package.json")], ...(await files("dist", "dist"))];
const local = [];
const central = [];
let offset = 0;
...
await rm("../../artifacts/fubun-vscode-extension.vsix", { force: true });
await writeFile("../../artifacts/fubun-vscode-extension.vsix", Buffer.concat([...local, centralBytes, end]));
